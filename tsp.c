...
struct AlgorithmState list_solutions[MAX_THREADS + 1][64];
...
{
    printf("Cost %f\nCities visited: %d\nPrev step %p \n", tour->cost, tour->cities_visited, tour->previous_step);
}
...
void print_result(struct AlgorithmState *algo_state)
{
    ...
    for (int i = 0; i < cities_visited - 1; i++)
        printf("%d ", values[i]);
    printf("0 \n");
}
...
inline double compute_updated_lower_bound(double lower_bound, double jump_cost, unsigned int source_city, unsigned int destination_city)
{
    ...
    double ct = (comp_1)*cities[source_city].min_cost2 + (comp_1 == 0) * cities[source_city].min_cost;
...
inline struct Tour *go_to_city(struct Tour *tour, short city_id, double lb, double cost)
{
...
void visit_city(struct Tour *tour, int destination, struct AlgorithmState *algo_state, int *tours_created)
{
    ...
    if (!get_was_visited(tour, i))
    {
        ...
        {
            struct Tour *new_tour = go_to_city(tour, i, new_lb, tour->cost + jump_cost);
            ...
            else
            {
                ...
                if (current_tour_is_better)
                {
                    ...
                    algo_state->solution = go_to_city(new_tour, 0, final_lb, final_cost);
...
void visit_city_parallel(struct Tour *th_tour, int destination, struct AlgorithmState *algo_state, priority_queue_t *th_queue, int *tours_created)
{
    int i = destination;
    int id = omp_get_team_num();
    if (!get_was_visited(th_tour, i))
    {
        double jump_cost = get_cost_from_city_to_city(th_tour->current_city, i);
        double new_lb = compute_updated_lower_bound(th_tour->lb, jump_cost, th_tour->current_city, i);
        if (new_lb <= algo_state->solution->lb)
        {
            struct Tour *new_tour = go_to_city(th_tour, i, new_lb, th_tour->cost + jump_cost);
            int finished = get_visited_all_cities(new_tour, algo_state);
            if (!finished)
            {
#pragma omp critical
            ...
            else
            {
                ...
                double final_lb = compute_updated_lower_bound(new_tour->lb, jump_cost, i, 0);
                double final_cost = new_tour->cost + jump_cost;
                int current_tour_is_better = final_cost < list_solutions[MAX_THREADS][0].solution->cost;
                if (current_tour_is_better)
                {
...
int analyseTourParallel(struct Tour *tour, struct AlgorithmState *algo_state)
{
    int tours_created = 0;
    int loops = algo_state->number_of_cities - 1;
    omp_set_num_threads(MAX_THREADS);
    struct Tour *pop_tour;
    priority_queue_t *th_queue[MAX_THREADS][64];
    for (int n = 0; n < MAX_THREADS; n++)
        th_queue[n][0] = queue_create((char (*)(void *, void *))NULL);
...
#pragma omp parallel
    {
        ...
        struct Tour *th_tour = (struct Tour *)get_clean_step();
        th_tour->current_city = tour->current_city;
        th_tour->cities_visited = tour->cities_visited;
        th_tour->lb = tour->lb;
        th_tour->cost = tour->cost;
        th_tour->previous_step = tour->previous_step;
...
void tscp(struct AlgorithmState *algo_state)
{
    ...
    {
        list_solutions[i][0].solution = (struct Tour *)get_clean_step();
        list_solutions[i][0].solution->cost = algo_state->max_lower_bound * 2;
        list_solutions[i][0].solution->cities_visited = algo_state->all_cities_visited_mask;
        list_solutions[i][0].solution->current_city = 0;
        list_solutions[i][0].solution->previous_step = NULL;
        list_solutions[i][0].solution->lb = algo_state->max_lower_bound;
    }
    ...
    {
        int newToursCreated = analyseTourParallel(current_tour, algo_state);
        ...
        /*
                ...
                if (it > 1300)
                {
                    printf("iteration meme time debug %d \n", it);
                    //((struct step_middle *)current_tour)->ref_counter = newToursCreated;
                    fflush(stdout);
                    printf("yee siteration meme time debug %d \n", it);
                }
                it = it + 1;
        */
    }
