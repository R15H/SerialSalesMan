...
void distribute_load(priority_queue_t *global, int number_of_cities)
{
    ...
    for (int i = 0; i < distributed_tours;)
    {
        ...
        if (!tour)
            break;
        queue_push(thread_states[atual].queue, tour);
        atual++;
        if (atual == nr_of_threads)
            atual = 0;
    }
}
...
{
#pragma omp parallel num_threads(12) shared(thread_states, algo_state)
    {
...
#pragma omp single
        {
            ...
            analyseTour(first_step, algo_state);
            for (int i = 0; i < nr_of_threads; i++)
            {
                thread_states[i].queue = queue_create((char (*)(void *, void *))NULL);
                thread_states[i].solution = (struct Tour *)get_clean_step();
                thread_states[i].solution->cost = algo_state->max_lower_bound * 2;
                thread_states[i].solution->cities_visited = algo_state->all_cities_visited_mask;
                thread_states[i].solution->current_city = 0;
                thread_states[i].solution->previous_step = NULL;
                thread_states[i].solution->nr_visited = algo_state->number_of_cities;
                thread_states[i].solution->lb = algo_state->max_lower_bound;
                thread_states[i].number_of_cities = algo_state->number_of_cities;               // TODO
                thread_states[i].all_cities_visited_mask = algo_state->all_cities_visited_mask; // TODO
                thread_states[i].max_lower_bound = algo_state->max_lower_bound;                 // TODO --> estas tralhas meter numa var que é readonly pra todas as gajs
            }

            distribute_load(algo_state->queue, algo_state->number_of_cities);
        }

        while (1)
        {
            execute_load(algo_state);
#pragma omp barrier
#pragma omp single
            {
                sync_q(algo_state->queue, algo_state);
            }
#pragma omp barrier
            if (algo_state->queue->size == 0)
                break;
#pragma omp barrier
#pragma omp single
            distribute_load(algo_state->queue, algo_state->number_of_cities);
