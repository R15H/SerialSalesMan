...
inline struct Tour *go_to_city(struct Tour *tour, short city_id, double cost)
{
...
void visit_city(struct Tour *tour, int destination, struct AlgorithmState *algo_state, int *tours_created)
{
    ...
    if (!get_was_visited(tour, i))
    {
        ...
        {
            struct Tour *new_tour = go_to_city(tour, i, new_cost);
            finished = get_visited_all_cities(new_tour, algo_state);
            ...
            else
            {
                ...
                if (current_tour_is_better)
                {
                    ...
                    algo_state->solution = go_to_city(new_tour, 0, final_cost);
...
void visit_city_parallel(struct Tour *tour, int destination, struct AlgorithmState *algo_state, int *tours_created)
{
    int finished;
    int i = destination;
    if (!get_was_visited(tour, i))
...
int analyseTourParallel(struct Tour *tour, struct AlgorithmState *algo_state)
{
    ...
    int tours_created = 0;
    int loops = algo_state->number_of_cities - 1;
    priority_queue_t *th_queue[MAX_THREADS];
...
#pragma omp parallel
    {
        ...
        th_queue[id] = queue_create((char (*)(void *, void *))NULL);
        struct AlgorithmState th_algo_state;
        struct Tour *th_tour = (struct Tour *)get_clean_step();
        struct Tour *aux_tour;
...
#pragma omp critical
        {
            ...
            th_algo_state.solution = (struct Tour *)get_clean_step();
            th_algo_state.solution->cost = algo_state->max_lower_bound;
            th_algo_state.solution->cities_visited = algo_state->all_cities_visited_mask;
            th_algo_state.solution->current_city = algo_state->solution->current_city;
            th_algo_state.solution->previous_step = algo_state->solution->previous_step;
            th_algo_state.queue = th_queue[id];
            th_algo_state.all_cities_visited_mask = algo_state->all_cities_visited_mask;
            th_algo_state.number_of_cities = algo_state->number_of_cities;
        }
#pragma omp barrier
...
#pragma omp for private(i)
        for (i = 0; i <= loops; i++)
        {
            //printf("Im th %d  in i:%d \n",id,i);
            visit_city_parallel(th_tour, i, &th_algo_state, &tours_created);
        }

#pragma omp critical
        {
            while ((aux_tour = queue_pop(th_queue[id])))
            {
                queue_push(algo_state->queue, aux_tour);
            }
            queue_delete(th_queue[id]);
        }
    }
    return tours_created;
}
